...
We'll then explain what our categorical constructs mean in the context of application to dynamical systems, and we'll end by showcasing implemented examples of these dynamical systems, some of which clearly display the advantages of this categorical framework.
...
One behavior is for one system to simulate another system, this can be encoded by a simple kind of commuting square consisting of a function between the states of the two different systems.
...
haltingEmitter A B = mkpoly (A ⊎ B) halting
    where halting : {A B : Set} → (A ⊎ B) → Set
          ...
          halting (inj₂ _) = ⊤
\end{minted}
    A system inside this interface takes no inputs in a different, more strict sense: it's not that it does not receive any outside information, like the \texttt{emitter}, but rather that any use of it must account for the case when an input cannot be produce.
...
One of the examples in the book is the tape of a Turing machine. A Turing machine consists of two components:
\begin{itemize}
    \item The infinitely long tape, indexed by integers, which: \begin{itemize}
...
A real Turing machine must be able to output something to the outer box, which happens once it's halted, while Figure \ref{wd:turing} only shows them while running/communicating. This is an example of mode-dependence being hard to express.
...
\end{minted}
And they're again parallel-productized, producing a system with 3 inputs and 3 outputs, then wired in the right way (x is given as an input to y and z, y to x and z, and z to y, as per the system equations)
...
A regular sum type, since we do not care about what the output of the system is until it's actually ready to predict.
