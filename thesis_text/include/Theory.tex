...
Sum, multiplication, exponentiation, and numeric literals, are all considered in the context of sets. A numeric literal such as $7$ represents the set $\underline{7}$, a set with 7 elements. The $+$ symbol corresponds to \textit{the categorical coproduct}, multiplication  corresponds to \textit{the categorical product} and exponentiation to the exponential object: in this context, it is the type of functions from the exponent to the base of the term. For example, $ f : 7^2 \cong f : 2 \rightarrow 7$. This way of thinking algebraically is very convenient, because plugging in "numbers" into the polynomial expressions to get "numbers" back will turn out to be a very useful kind of operation.
...
The identity lens leaves the polynomial untouched in both the positions and the direction at each position, which is implemented with the identity function.
...
For example, to prove the right unit law for the parallel product (explained later) $\otimes$ with Y as unit: \mintinline{agda}{rightUnit : (p : Polynomial) → p ⊗ Y ≡ p}.
...
This means that a proof that the map on positions is equal is needed, as well as a proof that the map on directions is equal.
Substitution is needed for the proof on the directions to make the types match.
...
This definition of equality is then transferred to the record definition of Polynomials leading to the final characterization of polynomial equality:
\begin{minted}{agda}
poly≡ : {a b : Polynomial}
...
In addition, a forall-quantified version of equality is provided, which simplifies some proofs:
\begin{minted}{agda}
poly≡∀ : {a b : Polynomial}
    → (fstA≡fstB : position a ≡ position b)
    → ((posB : position b) → 
...
In category theory, it is frequently of interest to know when two morphisms are equal. Therefore, a characterization of equality for lenses is also needed, and it should be convenient enough to be usable in practical proofs.
...
The reason is that \texttt{subst} is not very deep in the term, and the \texttt{subst} is of a complicated dependent function type, where the dependent function returns a new function with a varying domain.
Often, it is possible to use laws to push the \texttt{subst} operation further into the term, which makes lemmas easier to use.
This idea is captured by the more powerful variant of lens equality below, \mintinline{agda}{lens≡ₚ}. In it, \texttt{subst} is pushed further into the term making it more practical to use in proofs.
...
The final step is to show that this lensFromZero from $\mathbb{0}$ to $p$ is unique. This is done by assuming that there exists another lens $f$ from $\mathbb{0}$ to $p$, and then proving that $f$ actually is the same lens as \texttt{lensFromZero}. 
...
This is implemented by using the lens equality function and the fact that the absurd function (from the empty type) is unique, utilizing function extensionality. We then plug these proofs into the \texttt{agda-categories} characterization of the initial object:
...
Similarly to the initial object, to show that $\mathbb{1}$ is a terminal object amounts to first constructing a lens \textbf{from} every other object, and then showing that each of these lenses is unique.

The map on positions from any other polynomial to $\mathbb{1}$ is the function always returning \texttt{tt}, the unique inhabitant of the singleton set $\top$. The function itself is also often called \textit{unit}. For this position, the map on directions goes from $\bot$ since $\mathbb{1}$ has no directions, thus the map on directions is again the absurd function.
...
Finally, for uniqueness, any other lens $f$ from $p$ to $\mathbb{1}$ is the same as \texttt{lensToOne}. For map on positions equality, \texttt{refl} is used to show that any two functions (with same domain) to $\top$ is the same. For map on directions equality, uniqueness of the absurd function is used, as in the initial object. The more advanced version of lens equality is used to do the function extensionality in the background, allowing for more concise syntax.
...
\section{Quadruple adjunction with Set}
...
These functors turn out to form a chain of adjunctions: $L \dashv p(1) \dashv C \dashv p(0)$. Adjunctions are forms of weak equivalences between categories, given by pairs of functors: the left and right adjoints. The intuition is that the left adjoint "adds as much information" as possible to the objects and morphisms it acts on, whereas the right adjoint "removes as little information" as possible in order to make the objects and morphisms it acts on "fit" in its target category. In the case of the relationship between \textbf{Poly} and \textbf{Set}, there are two functors putting different copies of \textbf{Set} inside \textbf{Poly} (the fully faithful ones), and two functors that "forget" that they act on something \textit{containing} sets. 
...
\section{Comonoids in \textbf{Poly}($y$, $\triangleleft$) are small categories}

A fact that the book spends a lot of time on is that comonoids in the monoidal category \textbf{Poly}($\triangleleft$, $y$) are isomorphic to small categories (categories where the collection of objects forms a set). This means that within \textbf{Poly}, at least the objects of the category \textbf{Cat} of small categories exist, and indeed there is an unusual category of categories called \textbf{Cat$\sharp$} that can be modeled in \textbf{Poly}, where the morphisms are \textit{co}functors, the duals to functors. This is a deep fact, and suggests that \textbf{Poly} is a setting in which a considerable amount of category theory itself can be studied.

A comonoid in this monoidal category, as the dual to the monoid structure, consists of the following data:
\begin{itemize}
  \item A polynomial $c$, called the \textit{carrier}.
  \item A morphism $\delta : c \rightarrow c \triangleleft c$ called the the \textit{duplicator}.
  \item A morphism $\epsilon : c \rightarrow y$, called the \textit{eraser}.
\end{itemize}
And the following laws:
\begin{itemize}
  \item $\langle \text{id}_c \triangleleft \delta \rangle \circ \delta = \langle \delta \triangleleft \text{id}_c \rangle \circ \delta$, called the \textit{coassociativity} law. It is the dual to a monoid's associativity law, and means that there is no difference between duplicating, then duplicating on the left and duplicating, then duplicating on the right.
  \item $\sim^L = \langle \epsilon \triangleleft \text{id}_c \rangle \circ \delta$, the left counit law, saying that duplicating then erasing the left is the same as not doing anything.
  \item $\sim^R = \langle \text{id}_c \triangleleft \epsilon \rangle \circ \delta$, the right counit law, saying that duplicating then erasing the right is the same as not doing anything.
\end{itemize}

The lenses $\sim^R : c \rightarrow c \triangleleft y$ and $\sim^L : c \rightarrow y \triangleleft c$ are just putting its source polynomial (in this case the carrier) in the right or left of the $\triangleleft$ product with a $y$, but because $y$ is the monoidal unit for this product, this is roughly the same as not doing anything.

In Agda, the data is defined like the following type:
...
\begin{minted}{agda}
  ...
  field
    ...
    ε : Lens c Y
    δ : Lens c (c ◂ c)
    coassoc : ⟨ idLens {c} ◂ δ ⟩ ∘ₚ δ
              ≡
              transport (assoc⇆ {c}) (⟨ δ ◂ idLens {c} ⟩ ∘ₚ δ)
    leftCounit :  ~ᴸ ≡ ⟨ ε ◂ idLens {c} ⟩ ∘ₚ δ
    rightCounit : ~ᴿ ≡ ⟨ idLens {c} ◂ ε ⟩ ∘ₚ δ
\end{minted}

The \texttt{transport} uses the proof of associativity of the $\triangleleft$ operation to send the right-hand side to the same type as the left side. This proof is complicated, and makes this record hard to work with. We were therefore not able to complete this proof, but our attempt is included in the code. It consists of constructing an \texttt{Iso} value, with the two legs \texttt{comonoidsAreCategories : Category zero zero zero → Comonoid} and \texttt{categoriesAreComonoids : Comonoid → Category zero zero zero} being mutual inverses. The attempt includes most of the second leg and parts of the first leg.