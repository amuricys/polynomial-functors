...
x : DynamicalSystem
x = MkDynamicalSystem X (MkPolynomial X λ _ → Y) (readout ⇄ update)
  where readout : X → X
        readout state = state
        update : X → Y → X
...
y : DynamicalSystem
y = MkDynamicalSystem Y (MkPolynomial Y λ _ → X × Z) (readout ⇄ update)
  where readout : Y → Y
        readout state = state
        update : Y → X × Z → Y
...
z : DynamicalSystem
z = MkDynamicalSystem Z (MkPolynomial Z λ _ → X × Y) (readout ⇄ update)
  where readout : Z → Z
        readout state = state
        update : Z → X × Y → Z
