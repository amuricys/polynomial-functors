...
rest Reservoir (rdimf ∷ trainStepsf ∷ totalSeqStepsf ∷ lorinitx ∷ lorinity ∷ lorinitz ∷ dt ∷ []) = do
  ...
  let inputWeights = 𝕄 ((-0.064975537042022 Vec.∷ -0.065251741398635 Vec.∷ 0.058517783986069 Vec.∷ Vec.[]) Vec.∷ 
      ...
      resWeights = 𝕄 ((0.197316884195857 Vec.∷ -0.574663634125314 Vec.∷ 0.784674814076449 Vec.∷ Vec.[]) Vec.∷ 
                      ...
                      Vec.[])
      resVec = lorenzResList rdim trainSteps totalSeqSteps ( lorinitx , lorinity , lorinitz ) dt inputWeights resWeights
...
rest Reservoir _ = do 
  Level.lift tt ← IO.run {Level.zero} $ IO.putStrLn ("Error: missing parameters for reservoir")
  IO.run $ IO.pure tt
main : IO ⊤
main = do
